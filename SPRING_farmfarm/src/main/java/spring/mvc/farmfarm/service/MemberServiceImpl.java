...
public class MemberServiceImpl implements MemberService {
	...
	public void fundRanking(HttpServletRequest req, Model model) {
		ArrayList<RankingDTO> dtos1 = new ArrayList<>();
		ArrayList<RankingDTO> dtos2 = new ArrayList<>();
		dtos1=dao.getFundJoiner();
		dtos2=dao.getFundRanking();
		
		
		//순서정렬된 dtos1에다가 dots2의 값 넣어줌
		for(int i=0;i<dtos1.size();i++) {
			for(int j=0;j<dtos2.size();j++) {				
				if(dtos1.get(i).getFund_no().equals(dtos2.get(j).getFund_no())) {
					dtos1.get(i).setFund_title(dtos2.get(j).getFund_title());
					dtos1.get(i).setStock_name(dtos2.get(j).getStock_name());
					dtos1.get(i).setStock_price(dtos2.get(j).getStock_price());
					dtos1.get(i).setFund_price(dtos2.get(j).getFund_price());
					dtos1.get(i).setFund_endDate(dtos2.get(j).getFund_endDate());
					dtos1.get(i).setFarmer_id(dtos2.get(j).getFarmer_id());
					
				}
			}
		}
		
		model.addAttribute("dtos",dtos1);
	...
	public void auctionRanking(HttpServletRequest req, Model model) {
		ArrayList<RankingDTO> dtos1 = new ArrayList<>();
		ArrayList<RankingDTO> dtos2 = new ArrayList<>();
		dtos1=dao.getAucJoiner();
		dtos2=dao.getAucRanking();
