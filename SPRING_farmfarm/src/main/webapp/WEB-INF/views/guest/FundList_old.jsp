...
<body>
   ...
   <h3>
      <i class="fa fa-angle-right"></i> 펀드 종료내역
   </h3>
   <div class="row">
      <!-- 펀드 진행내역 시작 -->
                  ...
                  <c:if test="${oldCnt>0}">
                     <c:forEach var="dto" items="${oldDtos}">
                        <tr>
                           ...
                           <td><a href="FundProductsContent?fund_no=${dto.fund_no}">${dto.fund_title}</a></td>
         ...
         <c:if test="${cnt>0 }">
            ...
            <ul class="pagination pull-right">
               <c:if test="${startPage > pageBlock}">
                  <li class="disabled"><a
                     href="FundList_old?${startPage - pageBlock}"> <span
                        class="glyphicon glyphicon-chevron-left"></span></a></li>
               </c:if>

               <c:forEach var="i" begin="${startPage}" end="${endPage}">
                  <c:if test="${i == currentPage}">
                     <li class="active"><a href="FundList_old?pageNum=${i}">${i}</a></li>
                  </c:if>

                  <c:if test="${i != currentPage}">
                     <li><a href="FundList_old?pageNum=${i}">${i}</a></li>
                  </c:if>
               </c:forEach>

               <c:if test="${pageCount > endPage}">
                  <li><a href="FundList_old?pageNum=${startPage + pageBlock}"><span
